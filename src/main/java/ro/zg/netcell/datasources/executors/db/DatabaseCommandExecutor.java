 ...
 */
package ro.zg.netcell.datasources.executors.db;
...
public class DatabaseCommandExecutor implements CommandExecutor<SqlCommandResponse> {
	...
	Connection conn = (Connection)commandContext.getConnectionManager().getConnection();
	if (conn == null) {
	...
	if (logger.isDebugEnabled()) {
	    if (resp != null) {
		logger.debug("Command executed on " + resp.getRowsCount() + " records.");
	    } else {
