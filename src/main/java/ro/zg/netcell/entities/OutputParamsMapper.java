...
public class OutputParamsMapper extends GenericEntity<GenericNameValueContext>{
	...
	Map<String,String> mappings = (Map<String,String>)input.getValue("outputPrmMappings");
	GenericNameValueContext result = executor.execute(inputContext);
	
	for(Map.Entry<String, String> entry : mappings.entrySet()){
	    String key = entry.getKey();
	    String value = entry.getValue();
	    inputContext.put(new GenericNameValue(key,result.getValue(value)));
	}
	return inputContext;
    }
